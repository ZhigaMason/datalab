...
Mosaic plots show that standalone **default** feature gives no insight into data, due to its values being extremly skewed towards negative answer and both groups have similar proportions of people subscribed to deposit.
...
We will plot box-plots of numerical features against target variable.
...
Boxes-and-whispers of both classes are very similar with median 39 and 40 and almost identical means. Population that has not subscribed has slightly lower range.
...
This box-and-whispers graph follows path of **duration** with *yes* being stretched out into a year timespan, while *no* is smashed into zero. An important remark is that value of 0 indicates, that the call made is the first one. From positivity rates we know that most people would not subscribed. /*explain previous sentance*/  It is expected, that calling the same of next days does not change a situation, so *no* population is destined to have less delay between contacts.
